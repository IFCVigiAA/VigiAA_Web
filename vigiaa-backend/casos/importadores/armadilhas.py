...
def upload_armadilhas(request):
    ...
    try:
        ...
        for i in range(len(df_raw)):
            ...
            row_set = set(row)
            if ("tipo armadilha" in row_set) and ("latitude" in row_set) and ("longitude" in row_set):
                header_idx = i
                break

        if header_idx is None:
            return JsonResponse({
                "erro": "Não achei a linha de cabeçalho (preciso ver 'Tipo Armadilha', 'Latitude', 'Longitude').",
        ...
        if "tipo armadilha" not in df.columns or "latitude" not in df.columns or "longitude" not in df.columns:
            return JsonResponse({
        ...
        for _, row in df.iterrows():
            tipo = row.get("tipo armadilha")
            lat = row.get("latitude")
            lon = row.get("longitude")

            if pd.isna(tipo) or pd.isna(lat) or pd.isna(lon):
                continue

            Armadilha.objects.create(
                tipo_armadilha=str(tipo).strip(),
                latitude=float(lat),
                longitude=float(lon),
            )
            inseridos += 1
