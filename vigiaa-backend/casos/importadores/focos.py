...
def upload_focos(request):
    # 1. Recupera o arquivo de focos
    arquivo = (
        request.FILES.get("focos") 
        or (hasattr(request, 'data') and request.data.get("focos"))
    )
    ...
    if mapeamento_raw:
        try:
            ...
            print(f"⚠️ Erro ao converter JSON em focos.py: {e}", flush=True)

    print("\n" + "="*60, flush=True)
    print(">>> [IMPORTADORES/FOCOS.PY] MAPEAMENTO CAPTURADO:", mapeamento_dict, flush=True)
    print("="*60 + "\n", flush=True)
